<template>
  <div class="container " style="min-height: 597px;">
        ...
        <div class="block-item">
          ...
          <div class="select-group">
            <select class="js-province-selector" v-model="provinceValue">
              ...
              <option v-for="p in addressData.list" v-bind:value="p.value">{{p.label}}</option>
            </select>
            <select class="js-city-selector" v-model="cityValue">
              <option value="-1">选择城市</option>
              <option v-for="c in cityList" v-bind:value="c.value">{{c.label}}</option>
            </select>
            <select class="js-county-selector" name="area_code" data-code="" v-model="districtValue">
              <option value="-1">选择地区</option>
              <option v-for="d in districtList" v-bind:value="d.value">{{d.label}}</option>
            </select>
          </div>
        </div>
        <div class="block-item">
    ...
    <div class="block section js-save block-control-btn" v-on:click="add">
      <div class="block-item c-blue center">保存</div>
    </div>
    <div class="block section js-delete block-control-btn" v-show="type==='edit'" v-on:click="remove">
      <div class="block-item c-red center">删除</div>
    </div>
    <div class="block stick-bottom-row center js-save-default" v-show="type==='edit'"  v-on:click="setDefault">
